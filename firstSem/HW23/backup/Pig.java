...
/* To - do list:
   1. Include the y exception (Possibly treat y as a vowel when in front of a string?) (Done)
   2. Caps? (Possibly create a helper method that discerns if a String is all caps or not) (Done)
   3. Punctuation (Possibly create a helper method that removes the punctuation, and then add it back in after) 
   4. Translate an entire sentence into PigLatin translator (after all of the above is done, and possibly use the white space as the incrementation
*/
...
public class Pig
{
	...
	if( beginsWithVowel(w)) { 
		...
		int firstVowel = w.indexOf("y"); //find the index of the first vowel in the string
		String firstPart = w.substring(0, firstVowel); //find the section of the string up to the first vowel
		retStr += w.substring(firstVowel); // add everything after the first vowel
		retStr += firstPart; //add the rest of the string
		retStr += "ay";
