...
public Object [] setCol( int c, Object[] newCol )
{
        ...
        for (int row = 0; row < size(); row++) {
                // track the indices of the array
                oldCol[row] = _matrix[row][c-1];         //add the former element
                _matrix[row][c-1] = newCol[row];
        }
        return oldCol;
}    //O(n)
...
public void transpose()
{
        Object store;
        for (int i = 0; i < size(); i++) {
                // take the first i elements in the matrix
                for(int j = 0; j < i; j++) {
                        store = _matrix[i][j];
...
public static void main( String[] args )
{
        ...
        Object[] replaceRow = { "jason", "jaysen"};
        x.setRow( 1, replaceRow);
        System.out.println("after setRow: \n" + x);

        Object[] replaceCol = {"hasif", "hasef"};
        x.setCol(1, replaceCol);
        System.out.println("after setCol: \n" + x);
